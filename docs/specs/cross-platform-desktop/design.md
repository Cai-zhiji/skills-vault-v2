...
- 平台和架构标识；
- 用户主目录与 Codex / Claude Code / Lux Neo 目标目录；
- 可执行文件候选名和扩展名；
- 是否支持 symlink；
- 默认部署策略；
- 可用安装提供者；
...
- Codex / Claude Code 继续部署完整 Skill 目录；Lux Neo 把 `SKILL.md` 映射为 `$LUX_HOME/skills/<name>.md`（默认 `~/.lux_neo/skills/<name>.md`），完整 Skill 目录作为同名资源目录，并映射可选 watcher JSON；
